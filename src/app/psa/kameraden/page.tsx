...
export default function KameradenPage() {
  ...
  const detailKamerad = detailId ? kameraden.find((k) => k.id === detailId) : null;
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground">
        Kameraden werden im Portal verwaltet. Hier nur Anzeige mit Größen und zugewiesener Ausrüstung.
      </p>
      ...
      <div className="grid gap-2">
        {filtered.map((k) => {
          const count = ausruestung.filter((a) => a.kameradId === k.id && a.status === "Ausgegeben").length;
          return (
            <button
              key={k.id}
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  ...
                  {k.dienstgrad && (
                    <span className="text-xs text-muted-foreground ml-2">{k.dienstgrad}</span>
                  )}
                  {!k.aktiv && (
                    <span className="text-xs bg-red-500/10 text-red-500 ml-2 px-1.5 py-0.5 rounded">inaktiv</span>
                  )}
                </div>
                <span className="text-xs text-muted-foreground">{count} Stück</span>
              </div>
            </button>
          );
        })}
      ...
      {detailKamerad && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setDetailId(null)}>
          <div className="bg-card border border-border rounded-lg p-6 w-full max-w-lg max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <h3 className="font-semibold text-lg mb-4">
              {detailKamerad.vorname} {detailKamerad.name}
              {detailKamerad.dienstgrad && (
                <span className="text-sm text-muted-foreground ml-2">({detailKamerad.dienstgrad})</span>
              )}
            </h3>
            ...
            {/* Ausrüstung */}
            <div>
              <h4 className="text-sm font-medium mb-2">Zugewiesene Ausrüstung ({detailAusruestung.length})</h4>
              {detailAusruestung.length === 0 ? (
                <p className="text-sm text-muted-foreground">Keine Ausrüstung zugewiesen.</p>
              ) : (
                <div className="space-y-1">
                  {detailAusruestung.map((a) => (
                    <div key={a.id} className="flex justify-between text-sm py-0.5">
                      <span>
                        {a.ausruestungstyp}
                        {a.seriennummer && <span className="text-muted-foreground ml-1">#{a.seriennummer}</span>}
                      </span>
                      <span className="text-xs bg-muted px-1.5 py-0.5 rounded">{a.status}</span>
                    </div>
                  ))}
                </div>
              )}
