...
export default function TypenPage() {
  ...
  const [editItem, setEditItem] = useState<Partial<Ausruestungstyp> | null>(null);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-3">
        {typen.map((t) => (
          <div key={t.id} className="bg-card border border-border rounded-md p-4">
            <div className="flex items-start justify-between">
              <div>
                <div className="font-medium">{t.bezeichnung}</div>
                {t.typ && <span className="text-xs bg-muted px-1.5 py-0.5 rounded">{t.typ}</span>}
              </div>
              {user.canEdit && (
                <div className="flex gap-1">
                  <button onClick={() => setEditItem({ ...t })} className="p-1 hover:bg-muted rounded">
                    <Pencil className="h-3.5 w-3.5" />
                  </button>
                  <button onClick={() => handleDelete(t.id)} className="p-1 hover:bg-muted rounded text-red-500">
            ...
            <div className="mt-2 flex flex-wrap gap-3 text-xs text-muted-foreground">
              {t.pruefintervallMonate && <span>Prüfung: alle {t.pruefintervallMonate} Mon.</span>}
              {t.maxLebensdauerJahre && <span>Lebensdauer: {t.maxLebensdauerJahre} J.</span>}
        ...
        {typen.length === 0 && (
          <p className="text-sm text-muted-foreground text-center py-8">Keine Typen vorhanden.</p>
        )}
      </div>
      ...
      {editItem && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-card border border-border rounded-lg p-6 w-full max-w-md space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-semibold">{editItem.id ? "Typ bearbeiten" : "Neuer Typ"}</h3>
            ...
            <div className="space-y-3">
              ...
              <label className="block">
                <span className="text-xs text-muted-foreground">Kategorie (Typ)</span>
                <input
                  type="text"
                  value={editItem.typ || ""}
                  onChange={(e) => setEditItem({ ...editItem, typ: e.target.value })}
                  className="w-full mt-1 px-2 py-1.5 bg-background border border-border rounded-md text-sm"
              ...
              <label className="block">
                ...
                <select
                  value={editItem.norm || ""}
                  onChange={(e) => setEditItem({ ...editItem, norm: e.target.value || null })}
                  className="w-full mt-1 px-2 py-1.5 bg-background border border-border rounded-md text-sm"
                >
